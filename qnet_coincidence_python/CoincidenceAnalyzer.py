...
def main():
    ...
    #Container(RecvCapability(url,topic)).run()
    thread_receive_capability = Thread(target=create_receiver_capability, args=(url,topic))
    thread_receive_capability.start()

    topic='topic://'+capabilityData['endpoint']+'/specifications'
    thread_receive_specification = Thread(target=create_receiver_specification, args=(url,topic))
    thread_receive_specification.start()

    #Publish result in '/multiverse/qnet/coincidence/results' to the controller
    #background_thread_send_result = Thread(target=send_result)
    #background_thread_send_result.start()
...
# Publish self Capability, collect AgentTT Capabilities (subscription)
#...
# Subscribe to self Specification topic (callback)
#...


# Process Specification: Coincidences measurement
    # Pick up AgentTT Capabilities (given in Specification parameter)
    #...

    # if OK: send Receipt
    #...

    # Measure for calibration
    Atemp = AliceTTAgent.getTimeTagStream()     # send Specification to Alice's Agent, with cal_int_time
    Btemp = BobTTAgent.getTimeTagStream()       # send Specification to Bob's Agent, with cal_int_time

    # After receiving results, Process calibration
    Adata = TTdata(fn, Atemp, dicty["Alice"]["chan"], dicty["Alice"]["sync"], dicty["Alice"]["pps"], Calibrate = True)
    Acal = Adata.calibration

    Bdata = TTdata(fn, Btemp, dicty["Bob"]["chan"], dicty["Bob"]["sync"], dicty["Bob"]["pps"], Calibrate = True)
    Bcal = Bdata.calibration
    ...
    bin100 = coinc.bin100
    print('coincidence bin: %d'%bin100)
    print('Alice 1 pps: ', Adata.pps)
    print('Bob 1 pps: ', Bdata.pps)
    ...
    # Measure for coincidences
    for n in range(20): # number of measurements: parameter in analyzer Specification

        ##### Measure from each Agent #####
        # send Specification to Alice's Agent, with int_time
        #...
        # send Specification to Bob's Agent, with int_time
        #...

        ##### Process #####
        Adata = TTdata(fn, Atemp, dicty["Alice"]["chan"], dicty["Alice"]["sync"], dicty["Alice"]["pps"], Calibrate = False, calFactor=Acal)
        Bdata = TTdata(fn, Btemp, dicty["Bob"]["chan"], dicty["Bob"]["sync"], dicty["Bob"]["pps"], Calibrate = False, calFactor=Bcal)
        ...
        cval     = np.append(cval, cps)
        accval   = np.append(accval, acc)
        aliceval = np.append(aliceval, alice)
        bobval   = np.append(bobval, bob)
        ...
        fig.plot([coinc.acc_windw_1, coinc.acc_windw_1],[0, max(coinc.H)*1.1])
        fig.plot([coinc.acc_windw_2, coinc.acc_windw_2],[0, max(coinc.H)*1.1])
    
        fig.pause(0.05)
    
        #fig.plot(cval)
        #fig.title('coincidences: %d; s1: %d; s2: %d'%(cps,alice,bob))
        #fig.xlabel("polarization angle")
        #fig.pause(0.05)
