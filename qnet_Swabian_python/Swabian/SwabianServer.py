...
SERVER_HOSTNAME_OR_IP = '129.6.168.224'
...
@Pyro5.api.expose
class TimeTagStream(MeasurementBase):

     def getData(self):
        
        dat = (self._obj.getData())
        # data structure: 
        ...
        a = numpy.array(dat.getEventTypes(),dtype=numpy.uint64)
        b = numpy.array(dat.getMissedEvents(),dtype=numpy.uint64)
        c = numpy.array(dat.getChannels(),dtype=numpy.uint64)
        d = numpy.array(dat.getTimestamps(),dtype = numpy.uint64)
        
        # prepare 2, 64 bit packets and join those
        ttagData = numpy.vstack((numpy.left_shift(a,56) + numpy.left_shift(b,32) + c  ,d)).reshape((-1,),order='F')   
        
        return ttagData

@Pyro5.api.expose
class TimeTagStreamBuffer(MeasurementBase):

    def getTimeStamps(self):
        return self._obj.getTimeStamps()
    ...
    def getEventTypes(self):
        return self._obj.getEventTypes()

    def getMissedEvents(self):
        return self._obj.getMissedEvents()

    def hasOverflows(self):
        return self._obj.hasOverflows()
...
class TimeTaggerBase(IteratorBase):
    ...
    def setConditionalFilter(self, *args):
        return self._obj.setConditionalFilter(*args)
...
class TimeTaggerRPC:
    ...
    def TimeTagStream(self, tagger_proxy, *args, **kwargs):
        return self._instantiate(TimeTagStream, tagger_proxy, args, kwargs)
